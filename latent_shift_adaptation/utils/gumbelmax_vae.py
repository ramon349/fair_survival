...
class GumbelMaxVAE(tf.keras.Model):
  ...
  def __init__(
    ...
    self.var_x = tf.Variable(
        initial_value=var_x, name="var_x", trainable=False, dtype=tf.float32
    )
  ...
  def _parse_data(self, data):
    ...
    t = data[:, indices["t"][0] : indices["t"][1]]
    e = data[:, indices["e"][0]: indices['e'][1]]
    return x, y, c, w,t,e
  ...
  def train_step(self, data):
    with tf.GradientTape() as tape:
      ...
      x_rec, y_rec, c_rec, w_rec,_,_= self._parse_data(reconstruction)
