...
class Method(baseline.Method):
  ...
  def __init__(
    ...
    Args:
      ...
      decoder_width: Width of the models used in the decoder.
      ...
      latent_dim: dimension of the latent variable u
      compressed_latent_dim: dimension of the compressed/distilled latent space.
    ...
    #or model_xu2y.layers[-1].activation != linear_act its ok for y to output non linear outputs 
    ...
    if model_xu2y.input_shape != (None, dims.x + compressed_latent_dim):
      raise ValueError(f"input shape for model_xu2y is invalid. {dims.x} {compressed_latent_dim}")
    if model_xu2y.output_shape != (None, num_classes):
      raise ValueError(f"output shape for model_xu2y is invalid. {model_xu2y.output_shape} {num_classes}") 
  ...
  def predict_hazard(self,model_input,**kwargs):
    ...
    for category in range(self.compressed_latent_dim):
      ...
      result_temp[:, :, :,category] = (ST * pu * self.freq_ratio[category]).reshape(batch_len,1,time_points)
    # Sum over U
